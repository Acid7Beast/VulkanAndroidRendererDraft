...
VulkanPipeline::VulkanPipeline(VulkanDevice* device, VkFormat colorFormat, VkFormat depthFormat, AAssetManager* assetManager) : m_device(device) {
...
void VulkanPipeline::createRenderPass(VkFormat colorFormat, VkFormat depthFormat) {
...
void VulkanPipeline::createGraphicsPipeline(AAssetManager* assetManager) {
    auto vertShaderCode = readAsset(assetManager, "shaders/base.vert.spv");
    auto fragShaderCode = readAsset(assetManager, "shaders/base.frag.spv");
...
VkShaderModule VulkanPipeline::createShaderModule(const std::vector<uint8_t>& code) {
...
std::vector<uint8_t> VulkanPipeline::readAsset(AAssetManager* assetManager, const std::string& filename) {
    AAsset* asset = AAssetManager_open(assetManager, filename.c_str(), AASSET_MODE_BUFFER);
    if (!asset) throw std::runtime_error("failed to open asset: " + filename);
